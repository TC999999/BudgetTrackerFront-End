...
const ExpenseCard: React.FC<Props> = ({
  ...
  return (
    <div className="expense-card grid grid-cols-4 p-4">
      <div className="expense-title p-2 text-sm sm:text-base duration-150 text-center content-center">
        {expense.title}
      </div>
      <div className="expense-transaction p-2 text-red-700 text-sm sm:text-base duration-150 text-center content-center">
        -{dollarConverter(expense.transaction)}
      </div>

      <div className="expense-date p-2 text-sm sm:text-base duration-150 text-center content-center">
        <p>{dateTime.current.date}</p>
        <p>{dateTime.current.time}</p>
      </div>

      {"budget" in expense && "budgetID" in expense ? (
        <div className="expense-budget-title p-2 text-sm sm:text-base duration-150 text-center content-center">
          <Link
